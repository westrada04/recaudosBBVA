(function () {
    ...
    angular
        ...
        .factory('CreateModifyAgreementService', CreateModifyAgreementService);

    function CreateModifyAgreementService($q, $http, API_BACKEND, UserService, $sessionStorage) {
        ...
        function getGeneralData() {
            ...
            requestGeneralData.agreementNumber = agrement.data.idAgreement.substring(1, agrement.data.idAgreement.length - 3);
            ...
            angular.forEach(this.request.indicators.data, function (value, key) {
                switch (value.id) {
                    ...
                    case 'ASSOCIATE_AGREEMENT':
                        requestGeneralData.associatedAgreements = value.isActive;
                        requestGeneralData.statusAssociatedAgreements = value.isActive;
                        requestGeneralData.listStatusAssociatedAgreements = [];
                        if (requestGeneralData.associatedAgreements) {
                            angular.forEach(value.value, function (data, key) {
                                requestGeneralData.listStatusAssociatedAgreements.push({
                                    associatedAgreementCode: data.name.split('-')[1].trim(),
                                    numberAssociatedAgreement: data.name.split("-")[2].trim(),
                                    associatedAccount: data.name.split("-")[0].trim()
                                });
                            });
                        }
                        break;
            ...
            angular.forEach(listindicators.data, function (value, key) {
                switch (value.id) {
                    ...
                    case 'DESTINATION_OUTPUT_F':
                        requestOutputFile.destination = value.isActive ? value.value[0].name == "C" ? "CASH" : "FTP" : "";
                        break;
                ...
                switch (value.referenceType.name) {
                    case 'REF':
                        ...
                        requestreferences.references.push({
                            "referenceId": parseInt(value.referenceType.id),
                            ...
                            "fillCharacter": value.paddingCharacters == 1 ? 'ZEROS' : 'SPACES',
                            "taxOver": taxover,
                    ...
                    case 'VAL':
                        ...
                        break;
                    case 'MEN':
                        requestreferences.informationMessage = requestreferences.informationMessage + value.referenceDescription + value.longDescription;
                        break;
                }
            });
        ...
        function getCaptureType() {
            ...
            requestCaptureType.EANCode = this.request.agreements.data.EANCode;
